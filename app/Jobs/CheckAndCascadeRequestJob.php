...
class CheckAndCascadeRequestJob implements ShouldQueue
{
    ...
    public function __construct(
        ...
        public int $offset = self::BATCH_SIZE
    ) {}

    public function handle(DonorMatchingService $matchingService, TelegramService $telegram): void
    {
        ...
        try {
            ...
            $rankedDonors = Cache::get($this->rankCacheKey(), []);
            if (!is_array($rankedDonors) || $rankedDonors === []) {
                $rankedDonors = $matchingService->rankDonors($bloodRequest, 50)->values()->all();
                Cache::put($this->rankCacheKey(), $rankedDonors, now()->addHours(6));
            }

            $batch = array_slice($rankedDonors, $this->offset, self::BATCH_SIZE);
            if ($batch === []) {
                return;
            }

            $this->sendAlerts($bloodRequest, $batch, $telegram);

            $nextOffset = $this->offset + self::BATCH_SIZE;
            if (array_slice($rankedDonors, $nextOffset, self::BATCH_SIZE) !== []) {
                self::dispatch(
    ...
    {
        return "blood_request:cascade:lock:{$this->bloodRequestId}";
    }
    ...
    private function sendFcmToDonors($donors, BloodRequest $bloodRequest): void
    {
        ...
        try {
            ...
            Log::warning('Targeted FCM dispatch failed in cascade job', [
